...
class GraphWin(tk.Canvas):
    ...
    def _onKey(self, evnt):
        self.secondToLastKey = self.lastKey
    ...
    def checkKeys(self):
        """Return last 2 keys pressed or None if no key pressed since last call"""
        if self.isClosed():
            raise GraphicsError("checkKey in closed window")
        self.update()
        
        keys = [self.lastKey, self.secondToLastKey]
        self.secondToLastKey = self.lastKey
        self.lastKey = ""
        return keys
